/*
  # Fix Leaderboard Function - Color Scheme Type Mismatch

  1. Problem
    - Function returns color_scheme as jsonb
    - Database stores it as text[] array
    - Type mismatch causes query to fail

  2. Solution
    - Convert text[] to jsonb using to_jsonb()
    - Maintains compatibility with frontend code expecting jsonb/array
*/
...
BEGIN
  ...
  member_stats AS (
    SELECT 
      ...
      to_jsonb(hk.color_scheme) as equipped_kit_colors,
