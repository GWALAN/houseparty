/*
  # Fix apply_kit_to_house Function - Convert Array to JSONB
  
  1. Changes
    - Updates apply_kit_to_house function to convert color_scheme from text[] to jsonb
    - house_kits.color_scheme is text[] (e.g., {#FF5733, #33FF57})
    - house_customizations.kit_color_scheme is jsonb
    - Use to_jsonb() to properly convert the array
...
BEGIN
  ...
  -- Get kit details including rarity and color_scheme (as text array)
  ...
  -- Update house_customizations with kit ID, rarity, and colors (as jsonb)
  UPDATE house_customizations
  SET
    equipped_house_kit_id = p_kit_id,
    kit_rarity = v_kit_rarity,
    kit_color_scheme = v_kit_colors_jsonb,
    updated_at = now()
  ...
  IF NOT FOUND THEN
    ...
    VALUES (p_house_id, p_kit_id, v_kit_rarity, v_kit_colors_jsonb, now());
  END IF;
